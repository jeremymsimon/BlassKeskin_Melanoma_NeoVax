# PBMCs: Define "Existing" Clones {#sec-pbmc-define-existing-clones}
...
## Defintion of pre-existing clones in PBMCs
1. Quality control metric: the clone must have 3 or more UMI in at least one time point
2. >10x the lower limit of detection at the pre-treatment timepoint
3. Present at all time points after pre-treatment
...
## Implement expression filter: >10x lower limit at pre-treatment
```{r}
...
## Implement secondary expression filter: Present at all time points after pre-treatment
```{r}
...
## Save the "Existing" beta clonotypes
```{r}
