# PBMCs: Define "Post-Nivolumab" Clones {#sec-pbmc-define-postnivolumab-clones}
...
## Defintion of post-nivo expanded clones in PBMCs
...
## Implement timepoint filter: Clones must be below (technically at, given the earlier conversion of undetected values to the LOD) the limit of detection at the pre-treatment timepoint
```{r}
...
## Implement expression filter: >10x the lower limit of detection at the pre-vax timepoint
```{r}
...
## Implement secondary expression filter: Present at all time points after pre-vax
```{r}
...
## Save the "Post-Nivolumab" beta clonotypes
```{r}
